...
}: { space: any; onClose: () => void; onSaved: (updated: any) => void }) {
  const [name, setName]         = useState(space.name ?? '');
  const [desc, setDesc]         = useState(space.description ?? '');
  const [color, setColor]       = useState(space.color ?? '#1e3a8a');
  const [saving, setSaving]     = useState(false);
  const [error, setError]       = useState('');

  const COLOR_PRESETS = ['#1e3a8a', '#7c3aed', '#059669', '#d97706', '#dc2626', '#0891b2', '#db2777'];

  const submit = async () => {
    if (!name.trim()) { setError('Name is required.'); return; }
    setSaving(true); setError('');
    try {
      const { space: updated } = await api.updateSpace(space.slug, { name: name.trim(), description: desc.trim(), color });
      onSaved(updated);
      onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/40 backdrop-blur-sm">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <span className="font-semibold text-gray-900">Edit Space</span>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-brand-500 focus:border-transparent outline-none"
            />
          </div>
          <div>
            ...
            <textarea
              ...
              className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-brand-500 focus:border-transparent outline-none resize-none"
            />
          </div>
          <div>
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-gray-100">
          ...
          <button
            ...
            className="flex-1 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold disabled:opacity-50 flex items-center justify-center gap-1.5"
          >
            {saving ? <><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Saving…</> : <><Check className="w-3.5 h-3.5" /> Save</>}
          </button>
        </div>
      </div>
    </div>
    ...
    <AppShell>
      ...
      {showEdit && (
        <EditSpaceModal
          ...
          onSaved={(updated) => setSpace((s: any) => ({ ...s, ...updated }))}
        />
      )}
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6 text-center">
            ...
            <h3 className="text-base font-bold text-gray-900">Delete "{space.name}"?</h3>
            <p className="text-sm text-gray-500 mt-1.5 mb-5">
              This will permanently remove the space and all its posts. This cannot be undone.
            </p>
            <div className="flex gap-3">
              <button
            ...
            {isAdmin && (
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 rounded-full border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-colors"
                  title="Edit space"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 rounded-full border border-red-200 text-sm font-medium text-red-500 hover:bg-red-50 transition-colors"
                  title="Delete space"
                >
